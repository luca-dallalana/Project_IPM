...
function sortLegendas() {
    ...
    let legendasArray = legendas.getArray().map((row) => ({
        id: row[0],
        label: row[1],
    }));

    // Sort the array by the label property
    legendasArray.sort((a, b) => a.label.localeCompare(b.label));

    // Convert the array back to a p5.Table object
    legendas = new p5.Table();

    // Add columns
    legendas.addColumn("id");
    legendas.addColumn("label");

    // Add rows
    for (let [index, item] of legendasArray.entries()) {
        let newRow = legendas.addRow();
        newRow.setNum("id", index); // Set the id to the current index
        newRow.setString("label", item.label);
    }
}
...
function draw() {
    if (draw_targets && attempt < 2) {
        ...
        textAlign(CENTER);
        text(
            legendas.getString(trials[current_trial], 1),
            width / 2,
            height - 20
        );
    }
    ...
    if (draw_targets) {
        for (var i = 0; i < legendas.getRowCount(); i++) {
            ...
            if (targets[i].clicked(mouseX, mouseY)) {
                ...
                console.log(trials[current_trial]);
